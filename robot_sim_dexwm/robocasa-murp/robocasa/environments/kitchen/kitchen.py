...
class Kitchen(ManipulationEnv, metaclass=KitchenEnvMeta):
    ...
    def __init__(
        ...
        mode=0,  # mode 0: Eval and MG; mode 1:Teleop
        offset_deltas=[-0.1, 0.1, -0.2, 0.2, -0.3, 0.3, -0.4, 0.4],
        robot_height_delta=0.1,
        robot_y_offset=-0.45,
    ):
        ...
        print(f"self.use_distractors: {self.use_distractors}")
        self.delta_list = offset_deltas
        print("ROBOT_OFFSET_TO_OBJECT_IN_X: ", self.delta_list)
        self.robot_height_delta = robot_height_delta
        print("ROBOT_INIT_HEIGHT_DELTA: ", self.robot_height_delta)
        self.robot_y_offset = robot_y_offset
        print("ROBOT_OFFSET_TO_COUNTER_IN_Y_MIN: ", self.robot_y_offset)
    ...
    def _load_model(self):
        ...
        for robot in self.robots:
            ...
            # zeros_tail = [0.0] * (full_length - len(qpos_list))
            zeros_tail = [
                -0.22888404291178063,
                -1.2542673370642603,
                -0.7083814475698837,
        ...
        # Select the random y offset given the range
        offset_y_sampling = self.rng.uniform(self.robot_y_offset, -0.45)

        print(
            f"Using y offset to the kitchen counter when placing the robot: {offset_y_sampling}"
        )

        robot_base_pos, robot_base_ori = self.compute_robot_base_placement_pose(
            ref_fixture=ref_fixture,
            offset=[0.0, offset_y_sampling],  # offset=[0.0, -0.45]
        )
        robot_base_pos[2] += self.robot_height_delta
        ...
        if "obj" in object_placements:
            # if the robot forward axis is Y [wrt world] , we move along x (based on ENV)
            if robot_base_ori[2] > 3.15 or robot_base_ori[2] < -3.15:
                diff_x = object_placements["obj"][0][0] - robot_base_pos[0]

                if diff_x > 0:
                    print("Hitting condition where robot moves in +X direction")
                    robot_base_pos[0] += diff_x + self.delta_num
                elif diff_x < 0:
                    print("Hitting condition where robot moves in -X direction")
                    robot_base_pos[0] += (
                        diff_x + self.delta_num
                    )  # This seems intended, though adding a negative value?
            # if the robot forward axis is X [wrt to world](based on env) then we move along Y
            elif robot_base_ori[2] <= 3.15:
                diff_y = robot_base_pos[1] - object_placements["obj"][0][1]

                if diff_y > 0:
                    print("Hitting condition where robot moves in -Y direction")
                    robot_base_pos[1] -= diff_y - 0.1 + self.delta_num
                    # robot_base_pos[0] -= 0.1 if object_placements["obj"][0][0] < 0 else 0.1
                elif diff_y < 0:
                    print("Hitting condition where robot moves in +Y direction")
                    robot_base_pos[1] -= diff_y + self.delta_num
    ...
    def _reset_internal(self):
        ...
        # Determine the empty action type based on how the controller is being set
        for part_name, controller in self.robots[
            0
        ].composite_controller.part_controllers.items():
            # Only take care of right arm for now
            if part_name == "right" and controller.input_type == "absolute":
                print(
                    f"compute new reset action for {part_name} controller undder {controller.input_type} mode"
                )
                start_idx, end_idx = self.robots[
                    0
                ].composite_controller._action_split_indexes[part_name]
                action[start_idx:end_idx] = (
                    self.robots[0]
                    .composite_controller.part_controllers[part_name]
                    .delta_to_abs_action(action, goal_update_mode=None)
                )
                # Need to change the pose if it is right_base
                if controller.input_ref_frame == "right_base":
                    action[start_idx:end_idx][0:6] = P.get_ee_T_arm_base(
                        self.sim, action[start_idx:end_idx][0:6], mode="sim"
                    )
    ...
    def get_corners_aabb(self, obj_name):
        body_id = self.sim.model.body_name2id(f"{obj_name}_main")
        body_pos = self.sim.data.body_xpos[body_id]
        body_xmat = self.sim.data.body_xmat[body_id].reshape(3, 3)
        corners_body = np.array(self.objects[obj_name].get_bbox_points())
        corners_world = []
        for corner_body in corners_body:
            corner_world = body_pos + body_xmat @ corner_body
            corners_world.append(corner_world)
        corners_world = np.array(corners_world)
        return corners_world

    def obj_block_check(
        self, target_pos, camera_pos, dist_threshold=0.6, target_obj_name="obj"
    ):
        """Check if the object is occluded or not"""
        # Define the ray's direction and length
        ray_start = camera_pos
        ray_end = target_pos
        ray_direction = ray_end - ray_start
        model = self.sim.model._model  # <mujoco._structs.MjModel object>
        data = self.sim.data._data  # <mujoco._structs.MjData object>
        geomid = np.zeros(1, dtype=np.int32)
        # Cast the ray using mj_ray
        dist = mj.mj_ray(
            m=model,
            d=data,
            pnt=ray_start.astype(np.float64),
            vec=ray_direction.astype(np.float64),
            geomgroup=None,
            flg_static=1,
            bodyexclude=-1,
            geomid=geomid,
        ...
        occlusoin = False
        if dist >= 0.0:
            # Make sure the camera is not too closed the object, so putting
            # 0.6 threshold there for the distance. In addition, make sure
            # that the ray hits the target object, which is target_obj_name (e.g., "obj_g0")
            if (
                dist < dist_threshold
                or target_obj_name not in model.geom(geomid[0]).name
            ):
                occlusoin = True

        return occlusoin

    def get_bbox(self, camera_name, obj_name):
        corner_w_frame = self.get_corners_aabb(obj_name)
        extrinsic = CU.get_camera_extrinsic_matrix(self.sim, camera_name)
        # HxW from the user fixated values
        width, height = self.camera_width, self.camera_height
        intrinsic = CU.get_camera_intrinsic_matrix(self.sim, camera_name, height, width)
        pixel_coords = []
        for coords in corner_w_frame:
            coords = np.hstack([coords, 1])
            P_cam = np.linalg.inv(extrinsic) @ coords
        ...
        u_min, v_min = pixel_coords.min(axis=0)
        u_max, v_max = pixel_coords.max(axis=0)
        bbox_2d = [u_min, v_min, u_max, v_max]

        # Start to check if the bbox is being occluded
        # Get the target object location
        body_id = self.sim.model.body_name2id(f"{obj_name}_main")
        body_pos = self.sim.data.body_xpos[body_id]
        occlusoin = self.obj_block_check(
            body_pos, extrinsic[:3, 3], target_obj_name=obj_name
        )

        # Make sure the bbox_2d is in range
        if any(x < 0 for x in bbox_2d):
            occlusoin = True
        if bbox_2d[2] > width and bbox_2d[0] > width:
            occlusoin = True
        if bbox_2d[3] > height and bbox_2d[1] > height:
            occlusoin = True
    ...
    def _create_obj_sensors(self, obj_name, modality="object"):
        ...
        @sensor(modality=modality)
        def obj_bbox_aabb_robot0_robotview(obs_cache):
            return self.get_bbox("robot0_robotview", obj_name)

        @sensor(modality=modality)
        def obj_bbox_aabb_robot0_robotview_2(obs_cache):
            return self.get_bbox("robot0_robotview_2", obj_name)

        @sensor(modality=modality)
        def obj_bbox_aabb_gripper0_right_right_eye_in_hand(obs_cache):
            return self.get_bbox("gripper0_right_right_eye_in_hand", obj_name)
        ...
        sensors = [
            ...
            obj_to_eef_pos,
            obj_to_eef_quat,
            obj_bbox_aabb_robot0_robotview,
            obj_bbox_aabb_robot0_robotview_2,
            obj_bbox_aabb_gripper0_right_right_eye_in_hand,
        ]
        names = [
            f"{obj_name}_pos",
            f"{obj_name}_quat",
            ...
            f"{obj_name}_bbox_in_robot0_robotview",
            f"{obj_name}_bbox_in_robot0_robotview_2",
            f"{obj_name}_bbox_in_gripper0_right_right_eye_in_hand",
        ]
