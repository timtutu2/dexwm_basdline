...
from robocasa.environments.kitchen.single_stage.kitchen_pnp import PnPCounterTop as PT
...
class ManipulateDrawer(Kitchen):
    ...
    def check_base_and_offsets(self, base_pos, check_funcs, offsets=[0.1, -0.1]):
        """
        check collision with offset -> reason to do this because check_fxtr_contact is done with just base
        and sometimes arms collide on wall. To flag this, we check whether base + offset is also colliding and reject that
        pose at all costs.
        """
        base_pos = np.array(base_pos)

        # first ceck at curr pos
        if any(f(base_pos) for f in check_funcs):
            return True

        # check at offset pos
        for axis in range(3):
            for offset in offsets:
                pos_offset = base_pos.copy()
                pos_offset[axis] += offset
                if any(f(pos_offset) for f in check_funcs):
                    return True
        return False
    ...
    def _load_model(self):
        ...
        (
            ...
            ref_fixture=self.drawer, offset=(-x_ofs, -0.65)
        )
        ...
        # check with offset on left
        if not self.check_base_and_offsets(
            test_pos_left, [self.check_fxtr_contact, self.check_sidewall_contact]
        ):
            inits.append((robot_base_pos_left, robot_base_ori_left, "right"))
        ...
        # same for right side
        if not self.check_base_and_offsets(
            test_pos_right, [self.check_fxtr_contact, self.check_sidewall_contact]
        ):
    ...
    def _check_success(self):
        ...
        success = False
        # NOTE: Mode == 1 is specifically for teleop and other mode (0) if soe mimicgen and evals
        if self.mode == 1:
            # Initialize counter only if not present
            if not hasattr(self, "hold_joint_p_frames"):
                self.hold_joint_p_frames = 0
            ...
            for joint_p in door_state.values():
                if self.behavior == "open":
                    ...
                    if joint_p < 0.15:
                        ...
                        break
            if success:
                self.hold_joint_p_frames += 1
                print("Holding pose times:", self.mode, self.hold_joint_p_frames)
            else:
                self.hold_joint_p_frames = 0
            return self.hold_joint_p_frames >= 100

        else:
            # For other modes, check success immediately
            for joint_p in door_state.values():
                if self.behavior == "open":
                    if joint_p < 0.70:
                        print("Yet to open:", joint_p)
                        return False
                elif self.behavior == "close":
                    if joint_p > 0.15:
                        return False
            ...
            # Check if robot has moved beyond threshold
            moved = not PT.check_robot_movement(
                rinitial_pos, rfinal_pos, robot_initial_mat, robot_final_mat
            )
            if moved:
                print("Robot has moved. Task not successful.")
                return False
            return True
