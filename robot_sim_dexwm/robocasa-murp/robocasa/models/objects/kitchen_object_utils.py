...
USE_TEST_SET = int(
    os.getenv("USE_TEST_SET", "0")
)

print(
    f"USE_TEST_SET: {USE_TEST_SET}"
)
...
def prep_dataset(OBJ_CATEGORIES, filter=False, ref_set=None, group_all=None):
    """
    Function that prepares the dataset according to Test or train split
    """
    updated_obj_categories = {}
    for (name, kwargs) in OBJ_CATEGORIES.items():
        ...
        if filter and name in ref_set:
            print(f"name: {name}")
            if group_all is not None and name in group_all:
                group_all.remove(name)
            continue  # Skip this object
        updated_obj_categories[name] = {}
        # create instances
        if objaverse_kwargs is not None:
            objaverse_kwargs.update(common_properties)
            updated_obj_categories[name]["objaverse"] = ObjCat(
            ...
            if updated_obj_categories[name]["objaverse"].name in large_obj:
                updated_obj_categories[name]["objaverse"].scale *= 1.25
                updated_obj_categories[name]["objaverse"].density = 25
            else:
                updated_obj_categories[name]["objaverse"].scale *= 1.5
                updated_obj_categories[name]["objaverse"].density = 25

        if aigen_kwargs is not None:
            aigen_kwargs.update(common_properties)
            updated_obj_categories[name]["aigen"] = ObjCat(
                name=name, aigen_cat=True, **aigen_kwargs
            )
    return updated_obj_categories


# Load Train
with open(os.path.join(BASE_JSON_PATH, "train_categories.json"), "r") as f:
    OBJ_TRAIN_CATEGORIES = json.load(f)
with open(os.path.join(BASE_JSON_PATH, "train_groups.json"), "r") as f:
    OBJ_TRAIN_GROUPS = json.load(f)
# Load test
with open(os.path.join(BASE_JSON_PATH, "test_categories.json"), "r") as f:
    OBJ_TEST_CATEGORIES = json.load(f)
with open(os.path.join(BASE_JSON_PATH, "test_groups.json"), "r") as f:
    OBJ_TEST_GROUPS = json.load(f)

#Using Training set
OBJ_CATEGORIES = prep_dataset(OBJ_TRAIN_CATEGORIES)
OBJ_GROUPS = OBJ_TRAIN_GROUPS
# Using test set
OBJ_CATEGORIES_TEST = prep_dataset(
    OBJ_TEST_CATEGORIES,
    filter=True,
    ref_set=OBJ_CATEGORIES,
    group_all=OBJ_TEST_GROUPS["all"],
)
if USE_TEST_SET:
    OBJ_CATEGORIES = OBJ_CATEGORIES_TEST
    OBJ_GROUPS = OBJ_TEST_GROUPS
    print("Using test kitchen objects and removing train objects")
...
def sample_kitchen_object_helper(
    ...
    if isinstance(groups, str) and groups.endswith(".xml"):
        ...
        print("Num of valid categories: ", len(valid_categories))
