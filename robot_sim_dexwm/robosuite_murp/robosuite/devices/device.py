...
class Device(metaclass=abc.ABCMeta):
    ...
    def input2action(self, mirror_actions=False) -> Optional[Dict]:
        ...
        # breakpoint()
        # grasp= 0 if grasp elif preelse -1

        ac_dict = {}
        # populate delta actions for the arms (with zeros for both arms)
        ...
        if robot.is_mobile:
            ...
            arm_norm_delta = np.concatenate([dpos, drotation])   # x, y, z, roll, pitch, yaw
        ...
        if hasattr(gripper, "grasp_qpos"):
            ac_dict[f"{active_arm}_gripper"] = getattr(gripper, "grasp_qpos")[grasp]   # pre-defined joint angles for grasp, pre-grasp, etc.
        else:
    ...
    def get_arm_action(self, robot, arm, norm_delta, goal_update_mode="target"):
        ...
        if isinstance(robot.part_controllers[arm], OperationalSpaceController):   # this is used in teleop
