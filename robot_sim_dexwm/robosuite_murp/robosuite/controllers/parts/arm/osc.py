...
class OperationalSpaceController(Controller):
    ...
    def __init__(
        ...
        **kwargs,  # does nothing; used so no error raised when dict is passed with extra terms used previously
        ...
        self.kp[2] *= 2  # increase kp for z control
        self.kp[3:6] *= 10  # increase kp for orientation control
        self.kd = 2 * np.sqrt(self.kp) * damping_ratio
        self.kd[2] *= 2  # increase kd for z control
    ...
    def set_goal(self, action):
        ...
        if self.input_type == "delta":
            ...
            # scaled_delta = delta
            # print('scaled_delta', scaled_delta)
        ...
        # Else, interpret actions as absolute values
            ...
            if self.use_ori is True:
                ...
                goal_wuat = R.from_matrix(self.goal_ori).as_quat()
            else:
                self.goal_ori = self.compute_goal_ori(np.zeros(3))
        else:
    ...
    def compute_goal_pos(self, delta, goal_update_mode=None):
        ...
        if goal_update_mode == "desired":
            ...
            if self.input_ref_frame in ["base", "right_base"]:
    ...
    def compute_goal_ori(self, delta, goal_update_mode=None):
        ...
        if self.orientation_limits is not None:
            delta_clipped = np.clip(
                delta, self.orientation_limits[0], self.orientation_limits[1]
            )
            quat_err = T.axisangle2quat(delta_clipped)
            rotation_mat_error = T.quat2mat(quat_error)
        return goal_ori
    ...
    def run_controller(self):
        ...
        if self.interpolator_pos is not None:
            ...
            elif self.input_ref_frame == "right_base":

                ee_T_world_pos = self.sim.data.get_body_xpos("robot0_right_hand")
                ee_T_world_mat = self.sim.data.get_body_xmat("robot0_right_hand")
                right_base_T_world_pos = self.sim.data.get_site_xpos(
                    "robot0_right_base_center"
                )
                right_base_T_world_mat = self.sim.data.get_site_xmat(
                    "robot0_right_base_center"
                )

                ee_T_right_base_pos = self.goal_pos
                # breakpoint()
                ee_T_right_base_ori = self.goal_ori
                # print("EE T BASE POS", ee_T_right_base_pos)
                ee_T_right_base = T.make_pose(ee_T_right_base_pos, ee_T_right_base_ori)
                right_base_T_world = T.make_pose(
                    right_base_T_world_pos, right_base_T_world_mat
                )
                # print("RIght_base_T_world", right_base_T_world)
                ee_T_world = right_base_T_world @ ee_T_right_base
                desired_world_pos = ee_T_world[:3, -1]
                # print("Desired world pos", desired_world_pos)
            else:
                raise ValueError
        ...
        if self.interpolator_ori is not None:
            ...
            if self.input_ref_frame == "base":
                ...
                desired_world_ori = ee_T_world[:3, :3]
            else:
                raise ValueError
        ...
        # Clip torques to actuator limits
        joint_min_limits = np.array(
            [-2.7437, -1.7837, -2.9007, -3.0421, -2.8065, 0.5445, -3.0159]
        )
        joint_max_limits = np.array(
            [2.743, 1.7837, 2.9007, -0.151, 2.8065, 4.5169, 3.0159]
        )

        safety_margin = 0.05  # Add a safety margin
        joint_min_limits += safety_margin
        joint_max_limits -= safety_margin
        damping_factor = 0.1  # Small corrective force to apply when hitting limits

        joint_positions = self.joint_pos + self.joint_vel * (1 / self.control_freq)
        # for i in range(len(joint_positions)):
        #     # if joint_positions[i] < joint_min_limits[i] and self.joint_vel[i] < 0:
        #     #     self.torques[i] = damping_factor  # Apply a small corrective force
        #     # elif joint_positions[i] > joint_max_limits[i] and self.joint_vel[i] > 0:
        #     #     self.torques[i] = -damping_factor  # Apply a small corrective force
        #     if joint_positions[i] < joint_min_limits[i] or joint_positions[i] > joint_max_limits[i]:
        #         # Scale torque smoothly as joint approaches limits
        #         if joint_positions[i] < joint_min_limits[i]:
        #             print(f"Joint {i} is below min limit")
        #             self.torques[i] *= (joint_positions[i] - joint_min_limits[i]) / (self.joint_pos[i] - joint_min_limits[i])
        #         elif joint_positions[i] > joint_max_limits[i]:
        #             print(f"Joint {i} is above max limit, hitting singularity")
        #             self.torques[i] *= (joint_max_limits[i] - joint_positions[i]) / (joint_max_limits[i] - self.joint_pos[i])
    ...
    def reset_goal(self, goal_update_mode="achieved"):
        ...
        # print(f"reset_goal: self.goal_pos: { self.goal_pos}")
        assert goal_update_mode in ["achieved", "desired"]
        self._goal_update_mode = goal_update_mode
