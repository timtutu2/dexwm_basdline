...
class MobileRobot(Robot):
    ...
    def _load_base_controller(self):
        ...
        if self.part_controller_config.get(self.base) is None:
            controller_path = os.path.join(
                ...
                "controllers/config/{}.json".format(
                    self.robot_model.default_controller_config[self.base]
                ),
            )
            self.part_controller_config[self.base] = load_part_controller_config(
        ...
        self.part_controller_config[self.base][
            "naming_prefix"
        ] = self.robot_model.base.naming_prefix
        ...
        ref_base_joint_indexes = [
            self.sim.model.joint_name2id(x) for x in self.robot_model.base_joints
        ]
        ref_base_joint_pos_indexes = [
            self.sim.model.get_joint_qpos_addr(x) for x in self.robot_model.base_joints
        ]
        ref_base_joint_vel_indexes = [
            self.sim.model.get_joint_qvel_addr(x) for x in self.robot_model.base_joints
        ]
        ...
        low = self.sim.model.actuator_ctrlrange[
            self._ref_actuators_indexes_dict[self.base], 0
        ]
        high = self.sim.model.actuator_ctrlrange[
            self._ref_actuators_indexes_dict[self.base], 1
        ]
    ...
    def _load_torso_controller(self):
        ...
        if self.part_controller_config.get(self.torso) is None:
            controller_path = os.path.join(
                ...
                "..",
                "controllers/config/{}.json".format(
                    self.robot_model.default_controller_config[self.torso]
                ),
            )
            self.part_controller_config[self.torso] = load_part_controller_config(
                custom_fpath=controller_path
            )
        ...
        self.part_controller_config[self.torso][
            "naming_prefix"
        ] = self.robot_model.naming_prefix
        ...
        ref_torso_joint_indexes = [
            self.sim.model.joint_name2id(x) for x in self.robot_model.torso_joints
        ]
        ref_torso_joint_pos_indexes = [
            self.sim.model.get_joint_qpos_addr(x) for x in self.robot_model.torso_joints
        ]
        ref_torso_joint_vel_indexes = [
            self.sim.model.get_joint_qvel_addr(x) for x in self.robot_model.torso_joints
        ]
        ...
        low = self.sim.model.actuator_ctrlrange[
            self._ref_actuators_indexes_dict[self.torso], 0
        ]
        high = self.sim.model.actuator_ctrlrange[
            self._ref_actuators_indexes_dict[self.torso], 1
        ]
    ...
    def _load_head_controller(self):
        ...
        if self.part_controller_config.get(self.head) is None:
            controller_path = os.path.join(
                ...
                "..",
                "controllers/config/{}.json".format(
                    self.robot_model.default_controller_config[self.head]
                ),
            )
            self.part_controller_config[self.head] = load_part_controller_config(
                custom_fpath=controller_path
            )
        ...
        self.part_controller_config[self.head][
            "naming_prefix"
        ] = self.robot_model.naming_prefix
        ...
        ref_head_joint_indexes = [
            self.sim.model.joint_name2id(x) for x in self.robot_model.head_joints
        ]
        ref_head_joint_pos_indexes = [
            self.sim.model.get_joint_qpos_addr(x) for x in self.robot_model.head_joints
        ]
        ref_head_joint_vel_indexes = [
            self.sim.model.get_joint_qvel_addr(x) for x in self.robot_model.head_joints
        ]
        ...
        low = self.sim.model.actuator_ctrlrange[
            self._ref_actuators_indexes_dict[self.head], 0
        ]
        high = self.sim.model.actuator_ctrlrange[
            self._ref_actuators_indexes_dict[self.head], 1
        ]
    ...
    def setup_references(self):
        ...
        for arm in self.arms:
            ...
            self._ref_joints_indexes_dict[arm] = [
                ...
                for joint in self.robot_model.arm_joints[start:end]
            ]
            self._ref_actuators_indexes_dict[arm] = [
                self.sim.model.actuator_name2id(joint)
                for joint in self.robot_model.arm_actuators[start:end]
            ]
            ...
            if self.has_gripper[arm]:
                ...
                self._ref_joints_indexes_dict[self.get_gripper_name(arm)] = [
                    self.sim.model.joint_name2id(joint)
                    for joint in self.gripper_joints[arm]
                ]
                self._ref_actuators_indexes_dict[
                    self.get_gripper_name(arm)
                ] = self._ref_joint_gripper_actuator_indexes[arm]
        ...
        self._ref_actuators_indexes_dict[self.base] = [
            self.sim.model.actuator_name2id(actuator)
            for actuator in self.robot_model.base_actuators
        ]

        self._ref_actuators_indexes_dict[self.torso] = [
            self.sim.model.actuator_name2id(actuator)
            for actuator in self.robot_model.torso_actuators
        ]

        self._ref_actuators_indexes_dict[self.head] = [
            self.sim.model.actuator_name2id(actuator)
            for actuator in self.robot_model.head_actuators
        ]

        self._ref_joints_indexes_dict[self.base] = [
            self.sim.model.joint_name2id(joint)
            for joint in self.robot_model.base_joints
        ]

        self._ref_joints_indexes_dict[self.torso] = [
            self.sim.model.joint_name2id(joint)
            for joint in self.robot_model.torso_joints
        ]

        self._ref_joints_indexes_dict[self.head] = [
            self.sim.model.joint_name2id(joint)
            for joint in self.robot_model.head_joints
        ]
    ...
    def _create_base_sensors(self, modality):
            ...
            return np.array(
                self.sim.data.site_xpos[
                    self.sim.model.site_name2id(
                        self.robot_model.base.correct_naming("center")
                    )
                ]
            )

        @sensor(modality=modality)
        def base_vel(obs_cache):
            return np.array(
                [
                    self.sim.data.qvel[self.sim.model.get_joint_qvel_addr(joint)]
                    for joint in self.robot_model.base_joints
                ]
            )
        ...
        sensors = [base_pos, base_quat, base_vel]
        names = ["base_pos", "base_quat", "base_vel"]
        ...
        for arm in self.arms:
            ...
            def base_to_eef_pos(obs_cache, arm=arm):
                ...
                eef_quat = T.convert_quat(
                    self.sim.data.get_body_xquat(self.robot_model.eef_name[arm]),
                    to="xyzw",
                )
                eef_mat = T.quat2mat(eef_quat)
                base_mat = self.sim.data.get_site_xmat(
                    self.robot_model.base.correct_naming("center")
                )

                T_WA = np.vstack(
                    (np.hstack((base_mat, base_pos[:, None])), [0, 0, 0, 1])
                )
            ...
            def base_to_eef_quat_site(obs_cache, arm=arm):
                ...
                eef_mat_site = self.sim.data.site_xmat[self.eef_site_id[arm]].reshape(
                    (3, 3)
                )
                base_mat = self.sim.data.get_site_xmat(
                    self.robot_model.base.correct_naming("center")
                ...
                T_WB = np.vstack(
                    (np.hstack((eef_mat_site, eef_pos[:, None])), [0, 0, 0, 1])
                )
                T_AB = np.matmul(np.linalg.inv(T_WA), T_WB)
            ...
            names += [
                ...
                f"base_to_{pf}eef_quat_site",
            ]

        return sensors, names

    def enable_parts(
        self, right=True, left=True, torso=True, head=True, base=True, legs=True
    ):
