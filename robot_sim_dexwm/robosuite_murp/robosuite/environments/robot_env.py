    ...
    def _setup_observables(self):
        ...
        if self.use_camera_obs:
            ...
            for (cam_name, cam_w, cam_h, cam_d, cam_segs) in zip(
                ...
                cam_sensors, cam_sensor_names = self._create_camera_sensors(
                    cam_name, cam_w=cam_w, cam_h=cam_h, cam_d=cam_d, cam_segs=cam_segs, modality="depth"
                )
                sensors += cam_sensors
        ...
        if cam_d:
            ...
            def camera_depth(obs_cache):
                if depth_sensor_name in obs_cache:
                    depth = obs_cache[depth_sensor_name]
                    depth = get_real_depth_map(self.sim, depth)
                else:
                    depth = np.zeros((cam_h, cam_w, 1))
                return depth
