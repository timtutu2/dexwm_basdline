...
class PickPlace(ManipulationEnv):
    ...
    def _load_model(self):
        ...
        xpos = np.array(xpos) - np.array([0.4, 0.0, 0.0])
    ...
    def _check_success(self):
        ...
        #     z_lift_threshold = self.bin1_pos[2] + 0.2  # e.g., 0.9
        #     obj_z = obj_pos[2]
        #     self.objects_in_bins[i] = int(obj_z > z_lift_threshold)

        # # returns True if a single object is in the correct bin
        # if self.single_object_mode in {1, 2}:
        #     return np.sum(self.objects_in_bins) > 0
        # Focus on a specific object (e.g., the second object in self.objects)
        obj = self.objects[1]  # Access the second object
        obj_str = obj.name
        obj_pos = self.sim.data.body_xpos[self.obj_body_id[obj_str]]
        dist = min(
            [
        ...
        z_lift_threshold = self.bin1_pos[2] + 0.2  # e.g., 0.9
        obj_z = obj_pos[2]
        self.objects_in_bins[1] = int(obj_z > z_lift_threshold)  # Update only for the second object

        # Return success based on the single object
        return self.objects_in_bins[1] > 0
