...
class FocalHeatmapLoss(KeypointMSELoss):
    ...
    def forward(self,
        ...
        num_pos = pos_inds.float().sum([-2,-1]) # calculating this only along the spatial dimension so that we can multiply with valid_kp_mask later
        # if num_pos == 0:
        #     loss = -neg_loss.sum([-2,-1])
        # else:
        #     loss = -(pos_loss.sum([-2,-1]) + neg_loss.sum([-2,-1])) / num_pos

        no_pos_ind = torch.where(num_pos==0)[0] # checking for heatmaps with no positives

        loss = -(pos_loss.sum([-2,-1]) + neg_loss.sum([-2,-1])) / num_pos  # this loss will have some inf values as some num_pos are zero, this is corrected in the next couple of lines
        if no_pos_ind.shape[0]>0:
            loss[no_pos_ind] = -neg_loss[no_pos_ind].sum([-2,-1])
